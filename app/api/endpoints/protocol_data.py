...
def download_qc1_protocol_data_json(
    ...
    """
    iqvdata_json_file_path = crud.pd_protocol_data.generate_iqvdata_json_file(db, aidoc_id)
...
async def qc1_protocol_upload(*,
    ...
    try:
        ...
        if uploaded_file_name_split[1] == ".json":
            protocol_data_updation_status = crud.pd_protocol_qcdata.save_qc_jsondata_to_db(db, aidoc_id,
                                                                                           protocol_file_path)
        elif uploaded_file_name_split[1] == ".xlsx":
            protocol_data_updation_status = crud.pd_protocol_qcdata.save_qc_exceldata_to_db(db, aidoc_id,
                                                                                            protocol_file_path)
