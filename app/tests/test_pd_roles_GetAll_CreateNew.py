...
@pytest.mark.parametrize("insert_flag, roleName, roleDescription, expected_response",
                         [("0","normalrole","normaldescription", status.HTTP_422_UNPROCESSABLE_ENTITY),
                          ("0", "", "description", status.HTTP_422_UNPROCESSABLE_ENTITY),
                          ("1", "newuser", "userfortestingpostmethod", status.HTTP_200_OK)])
def test_create_new_role(insert_flag, roleName, roleDescription, expected_response, new_token_on_headers):
    if insert_flag == "1":
