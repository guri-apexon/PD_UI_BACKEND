...
from app.qc_ingest.model.iqvsectionlock_db import IqvsectionlockDb
...
def test_create_labdata(doc_id, user_id,status_code, new_token_on_headers):
    ...
    if get_doc_status.status_code == 200:
        if user_id and db.query(IqvsectionlockDb).filter(IqvsectionlockDb.doc_id == doc_id).count():
