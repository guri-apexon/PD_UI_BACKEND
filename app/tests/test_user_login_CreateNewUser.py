...
@pytest.mark.parametrize("insert_flag, username, first_name,last_name,email,country,user_type, change_reason, expected_response",
                         [("0", "class101", "CS", "IT", "cs.it@iqvia.com", "USA", "normal", "ViaTicket", status.HTTP_409_CONFLICT),
                          ("1", "casio", "Casio", "Watch Brand IN", "casio.watchbrand@iqvia.com", "India", "normal", "ViaTicket", status.HTTP_200_OK),
                          ("0", "newclass", "", "new", "", "India", "", "", status.HTTP_422_UNPROCESSABLE_ENTITY)
                          ])
def test_create_add_new_user(insert_flag, username, first_name, last_name, email, country,  user_type, change_reason, expected_response, new_token_on_headers):
    if insert_flag == "1":
    ...
    create_user_in_login_user = client.post("api/create_new_user/new_user",
                                            json={"username": username, "first_name": first_name,
                                                  ...
                                                  "user_type": user_type, "reason_for_change": change_reason},
